...
class HttpClient:
    ...
    def _read(self, request: Request, *, stage: str) -> tuple[bytes, str, str | None]:
        ...
        for attempt in range(3):
            ...
            try:
                with urlopen(request, timeout=self.timeout, context=self.context) as response:
                    raw = response.read()
                    self.request_log.append({"stage": stage, "method": request.get_method(), "url": url,
                                             "attempt": attempt + 1, "elapsed_ms": round((time.perf_counter() - started) * 1000),
                                             "outcome": "ok", "http_status": response.status})
                    return raw, response.headers.get_content_type(), response.headers.get_content_charset()
            except HTTPError as exc:
                self.request_log.append({"stage": stage, "method": request.get_method(), "url": url,
                                         "attempt": attempt + 1, "elapsed_ms": round((time.perf_counter() - started) * 1000),
                                         "outcome": "error", "error_type": type(exc).__name__, "http_status": exc.code})
                if exc.code != 429 and exc.code < 500:
                    raise CollectionError(f"{stage}: {url}: HTTP {exc.code}") from exc
                error: Exception = exc
        ...
        raise CollectionError(f"{stage}: {url}: failed after 3 attempts: {reason}") from error
    ...
    def post_json(self, url: str, payload: dict[str, Any], headers: dict[str, str], *, stage: str = "request") -> bytes:
        ...
        raw, media_type, _ = self._read(request, stage=stage)
    ...
    def get(self, url: str, headers: dict[str, str] | None = None, *, stage: str = "request") -> tuple[bytes, str, str | None]:
        ...
        return self._read(request, stage=stage)
